...
//Создаем константы для работы с LOCAL STORAGE
const LOCAL_STORAGE_NEW_PRODUCT_ITEM = 'newProductItems';
const LOCAL_STORAGE_NEW_PRODUCT_OBJECT = 'objectProduct';
const LOCAL_STORAGE_NEW_INFO_ORDER_OBJECT = 'ObjectsInfoOrders'
...
const ordrerWrapper = document.querySelector('.orders-list-wrapper');
const myOrdersBtn = document.querySelector('#my-orders-menu');
...
const productInfoCard = function renderProductInfo(i, j) {
  ...
  data[i].products[j].dateBuy = new Date().toLocaleString();

  localStorage.setItem('objectProduct', JSON.stringify(data[i].products[j])); 
};
...
const infoObjectOrder = function getObjectFromInputOrder() {
  ...
  addNewObjectOrderInfoToLocalStorage(userData);

  return userData;  
};

//Создаем Функцию добавления объекта информации о заказе в LOCAL STORAGE
function addNewObjectOrderInfoToLocalStorage(item) {
  const newObjectOrderInfoItems = localStorage.getItem(LOCAL_STORAGE_NEW_INFO_ORDER_OBJECT);
  const updateObjectOrderInfoItems  = newObjectOrderInfoItems ? [item, ...JSON.parse(newObjectOrderInfoItems)] : [item];

  localStorage.setItem(LOCAL_STORAGE_NEW_INFO_ORDER_OBJECT, JSON.stringify(updateObjectOrderInfoItems));
}
...
//Вешаем события на кнопки SAVE и CANCEL
forma.addEventListener('submit', () => {
  if (validation(forma)) {
    ...
    const objectfromLocalStorage = localStorage.getItem(LOCAL_STORAGE_NEW_PRODUCT_OBJECT); 
    addNewProductItemToLocalStorage(JSON.parse(objectfromLocalStorage));
  };
});
...
// создаем функцию  добавления нового заказа в  LOCAL STORAGE
function addNewProductItemToLocalStorage(item) {
  const newProductItems = localStorage.getItem(LOCAL_STORAGE_NEW_PRODUCT_ITEM);
  const updateProductItems = newProductItems ? [item, ...JSON.parse(newProductItems)] : [item];

  localStorage.setItem(LOCAL_STORAGE_NEW_PRODUCT_ITEM, JSON.stringify(updateProductItems));

};

//вешаем события на кнопку  МОИ ЗАКАЗЫ
...
//создаем функцию рендера списка заказов
function renderOdrerList() {
  const objectItemOrderInfo = JSON.parse(localStorage.getItem(LOCAL_STORAGE_NEW_INFO_ORDER_OBJECT));
  const infoAboutOrderList = JSON.parse(localStorage.getItem(LOCAL_STORAGE_NEW_INFO_ORDER_OBJECT));
  const orderList = JSON.parse(localStorage.getItem(LOCAL_STORAGE_NEW_PRODUCT_ITEM));

  if(orderList) {
    const titleBlock = document.createElement('h1');
    ordrerWrapper.style.display = 'flex';
    oredBlockForm.innerHTML = '';    
    infoOrderBay.innerHTML = '';
    wrapperBlocks.innerHTML = '';
    ordrerWrapper.innerHTML = '';
    orderList.forEach((item,index) => {
      const orderCadr = document.createElement('div');
      ...
      const orderRemoveBtn = document.createElement('h4');
      
      titleBlock.classList.add('order-list-titleBlock');
      orderCadr.classList.add('order-list-card');
      orderNumber.classList.add('order-list-number');
      orderImg.classList.add('order-list-img');
      orderTitle.classList.add('order-list-title');
      orderPrice.classList.add('order-list-price');
      orderDate.classList.add('order-list-date');
      orderRemoveBtn.classList.add('order-list-remove');
      orderCadr.append(orderNumber);
      orderCadr.append(orderTitle);
      orderCadr.append(orderPrice);
      orderCadr.append(orderDate);
      orderCadr.append(orderRemoveBtn);
      orderRemoveBtn.innerText = 'x';
      
      titleBlock.innerText = 'MY ORDERS';
      ordrerWrapper.prepend(titleBlock);
      ordrerWrapper.append(orderCadr);
      orderNumber.innerHTML = `${index + 1}`;
      orderTitle.innerHTML = `${item.name}`;
      orderPrice.innerHTML = `${item.price} UAH`;
      orderDate.innerHTML = `${item.dateBuy.toLocaleString().replace(',', ' - ')}`;

      orderRemoveBtn.addEventListener('click', () => {
        orderList.splice(index, 1);
        infoAboutOrderList.splice(index, 1);

        localStorage.setItem(LOCAL_STORAGE_NEW_INFO_ORDER_OBJECT, JSON.stringify(infoAboutOrderList));
        localStorage.setItem(LOCAL_STORAGE_NEW_PRODUCT_ITEM, JSON.stringify(orderList));
        
        renderOdrerList(index);
      });

      orderTitle.addEventListener('click', () => {
        table.innerHTML = '';

        renderTableInfoOredrAtListOrders(objectItemOrderInfo[index], orderCadr);
      });
...
// создаем функцию рендера деталей заказа при клике на заказ
const table = document.createElement('table');

function renderTableInfoOredrAtListOrders(object, blockRender) {
  const rowHead = document.createElement('tr');
  blockRender.appendChild(table);
  rowHead.innerHTML = `<td colspan="2"><b>Info Order order and delivery</b></td>`;
  table.appendChild(rowHead);
